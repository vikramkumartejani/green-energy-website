  ...
  return (
    <div className='py-10 w-full bg-[#F4F9F0] px-3 md:px-7'>
      <div className='flex items-center justify-center flex-col gap-1'>
        ...
        <h3 className='text-[#103B38] text-[25px] sm:text-[33px] leading-[55px] font-semibold font-archivo text-center'>SCOPE OF WORK</h3>
        <p className='text-[#666666] text-[15px] sm:text-[22px] leading-[22px] sm:leading-[27px] text-center font-normal font-inter w-full max-w-[1116px] mx-auto'>Green Energy has made significant strides  in the Saudi market by assembling a team of  highly skilled specialists in their respective  fields, complemented by the expertise of our  international partners with extensive  experience in these domains.</p>
      </div>
      ...
      <div className="mt-16 w-full max-w-[1660px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-9">
        {boxesData.map((box, index) => (
          <div key={index} className="relative rounded-[40px] overflow-hidden px-3 xl:px-6 min-h-[525px] pt-12">
            ...
            ></div>
            <div className="relative z-20 xl:p-4 text-white text-center">
              <h2 className="text-[36px] leading-[24px] font-semibold mb-10">{box.title}</h2>
              <p className="text-[15px] xl:text-[18px] leading-[22.5px] font-normal mb-5">{box.description}</p>
              <p className="text-[15px] xl:text-[18px] leading-[22.5px] font-normal">{box.details}</p>
            </div>
          </div>
        ))}
      </div>
